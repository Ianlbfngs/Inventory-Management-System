...
export default function Login() {
    ...
    const verifyBackendStatus = async () => {
        try {
            ...
            try {
                await axios.get('http://localhost:8080/api/credentials/actuator/health');
    ...
    const onSubmit = async (e) => {
        ...
        if (await verifyBackendStatus()) {
            try {
                const result = await axios.post('http://localhost:8080/api/credentials/register', credential);
                alert("Register successful. Now please do the login");
                console.log(result);
                navigate("/login");
            } catch (error) {
        ...
        <div className='container'>
            ...
            <form onSubmit={(e) => onSubmit(e)}>
                ...
                <div className='text-center'>
                    ...
                    <Link className='btn btn-warning btn-m' style={{margin: "30px"}} to={"/login"}>Go back to login</Link>
